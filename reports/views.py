...
def _build_monthly_sales_data(year, month):
    ...
    # SalesOrder uses 'created_at' — there is no 'payment_date' on this model
    orders = SalesOrder.objects.filter(
        created_at__gte=start,
        created_at__lte=end
    ).select_related('fabric', 'created_by')

    # Payment correctly uses 'payment_date'
    payments = Payment.objects.filter(
        payment_date__gte=start,
        payment_date__lte=end
    ).select_related('sales_order', 'received_by')

    # DispatchTracking uses 'dispatch_date' — there is no 'payment_date' on this model
    dispatch = DispatchTracking.objects.filter(
        dispatch_date__gte=start,
        dispatch_date__lte=end
