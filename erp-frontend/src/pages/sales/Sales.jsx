...
import { buildDateParams } from '../../components/common/DateFilter';
...
export default function Sales() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [ord, dis, pay, fab, usr, sum] = await Promise.allSettled([
        api.get(`sales/orders/${buildDateParams(orderDateFilter)}`),
        api.get(`sales/dispatch/${buildDateParams(dispatchDateFilter)}`),
        api.get(`sales/payments/${buildDateParams(paymentDateFilter)}`),
  ...
  useEffect(() => { fetchAll(); }, [orderDateFilter, dispatchDateFilter, paymentDateFilter]);
  ...
  const filteredOrders = orders.filter(o => {
    ...
    return matchStatus && matchPay && matchBuyer;
  });
  const filteredDispatches = dispatches.filter(d => !dispatchStatusFilter || d.dispatch_status === dispatchStatusFilter);
  const filteredPayments   = payments.filter(p => !paymentMethodFilter   || p.payment_method  === paymentMethodFilter);
      ...
      {loading ? (
        ...
        {activeTab==='orders' && (
          <div className="space-y-3">
            {/* Combined single-line filter bar */}
            <div className="bg-white dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/60 px-4 py-3 flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Order:</span>
                {['','Draft','Confirmed','Dispatched','Completed','Cancelled'].map(s => (
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Pay:</span>
                {['','Pending','Partial','Paid'].map(s => (
                  <button key={s} onClick={() => setOrderPayFilter(s)}
                    className={`px-2.5 py-0.5 rounded-full text-xs font-medium transition-all ${
                      orderPayFilter === s
                        ? 'bg-blue-600 text-white shadow-sm'
                        : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'
              ...
              {/* Date dropdown */}
              <select
                value={orderDateFilter.type}
                onChange={e => setOrderDateFilter({ type: e.target.value })}
                className="border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg px-3 py-1.5 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors cursor-pointer flex-shrink-0"
              >
                {[{v:'all',l:'All Time'},{v:'today',l:'Today'},{v:'week',l:'This Week'},{v:'month',l:'This Month'},{v:'year',l:'This Year'}].map(o => (
              ...
              <div className="relative flex-shrink-0">
                ...
                />
                {buyerSearch && (
                  <button onClick={() => setBuyerSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
